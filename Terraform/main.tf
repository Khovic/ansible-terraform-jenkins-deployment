...
resource "aws_security_group" "myapp-sg" {
    ...
    egress {
        # allows outside access from vpc to any port on any address and protocol
        from_port = 0
        to_port = 0
        protocol = "-1"
        #source ips :
...
#to aquire the latest amazon linux image
...
resource "aws_instance" "myapp-server" {
    ...
    key_name = "poopy-oracle" # create key on aws webui
